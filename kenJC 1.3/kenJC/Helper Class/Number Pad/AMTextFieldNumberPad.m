	...
	if (windowTemp) {
		...
		if (self.keyboardAppearance == UIKeyboardAppearanceDefault) {
			if ([[[UIDevice currentDevice] systemVersion] hasPrefix:@"3"]) { 
				...
				[buttonDone setBackgroundImage:[UIImage imageNamed:@"search_bg1.png"] forState:UIControlStateNormal];
...
- (void)orientationDidChange:(NSNotification *)theNotification {
	...
	buttonRectShow = (isPortrait) ? CGRectMake(0.00f, [[UIScreen mainScreen] bounds].size.height - 53.0, 105.00f, 53.00f) : CGRectMake(0.00f, 281.00f, 158.00f, 39.00f);
	buttonRectHide = (isPortrait) ? CGRectMake(0.00f, [[UIScreen mainScreen] bounds].size.height - 53.0, 105.00f, 53.00f) : CGRectMake(0.00f, 443.00f, 158.00f, 39.00f);
...
//		[UIView setAnimationDidStopSelector:@selector(but021tonAnimationHide)];
//		[buttonDone setFrame:buttonRectHide];
//		[UIView commitAnimations];
	}
}
...
{
    if(textField.tag==189 || textField.tag==159)
    {
        NSUInteger newLength = [textField.text length] + [string length] - range.length;
        if(textField.tag==189)
            return (newLength > 11) ? NO : YES;
        else if(textField.tag==159)
